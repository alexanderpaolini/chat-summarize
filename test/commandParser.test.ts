...
describe('parseCommandOptions', () => {
  describe('query extraction', () => {
    it('should extract query from mention with text', () => {
      const result = parseCommandOptions(
        '<@123456789> what did James say about Fuji'
      );
  ...
  describe('flags with query', () => {
    it('should extract query and amount flag', () => {
      const result = parseCommandOptions(
        'chat summarize what did they say --amount 50'
      );
    ...
    it('should extract query and allow-summarizer flag', () => {
      const result = parseCommandOptions(
        '<@123456789> tell me about the meeting -S'
      );
    ...
    it('should handle both flags with query', () => {
      const result = parseCommandOptions(
        'chat summarize what happened -S -N 100'
      );
  ...
  describe('model flag', () => {
    ...
    it('should parse model flag with short form', () => {
      const result = parseCommandOptions(
        'chat summarize -M google/gemini-2.5-flash-lite'
      );
      expect(result.model).toBe('google/gemini-2.5-flash-lite');
      expect(result.query).toBeUndefined();
    });
    ...
    it('should parse model flag with query', () => {
      const result = parseCommandOptions(
        'chat summarize what happened --model google/gemini-2.5-flash-lite'
      );
      expect(result.model).toBe('google/gemini-2.5-flash-lite');
      expect(result.query).toBe('what happened');
    });
    ...
    it('should handle model flag with other flags', () => {
      const result = parseCommandOptions(
        'chat summarize -S --amount 50 -M google/gemini-2.5-flash-lite'
      );
      expect(result.allowSummarizer).toBe(true);
      expect(result.amount).toBe(50);
      expect(result.model).toBe('google/gemini-2.5-flash-lite');
    ...
    it('should handle model flag with query and other flags', () => {
      const result = parseCommandOptions(
        'chat summarize what did they say -S -N 100 --model google/gemini-2.5-flash-lite'
      );
      expect(result.query).toBe('what did they say');
      expect(result.allowSummarizer).toBe(true);
      expect(result.amount).toBe(100);
